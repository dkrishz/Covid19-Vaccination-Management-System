...
$query=mysqli_query($con,"update tblvacrecords set ReportStatus='$status',AssigntoName='$aname',AssignedtoEmpId='$pid',AssignedTime='$assigntime' where id='$testid'");
echo '<script>alert("Assigned to Phlebotomist successfully.")</script>';
echo "<script>window.location.href='phel-assigned-vaccine.php'</script>";
    }
...
$query2=mysqli_query($con,"update tblvacrecords set ReportStatus='$status',FinalReport='$newreportfile',ReportUploadTime='$uploadtime' where OrderNumber='$orderid'");
...
$query=mysqli_query($con,"insert into tblreporttracking(OrderNumber,Status,Remark,RemarkBy) values('$orderid','$status','$remark','$rby')");
$query2=mysqli_query($con,"update tblvacrecords set ReportStatus='$status' where OrderNumber='$orderid'");
echo '<script>alert("Status updated successfully")</script>';
echo "<script>window.location.href='phel-vaccine-status.php'</script>";
endif;  
...
<?php if($row['AssignedtoEmpId']==''): 
    ?>
      ...
      <button type="button" class="btn btn-dark btn-sm" data-toggle="modal" data-target="#assignto">Assign To</button>
            </div>                                        
<?php else: 
...
<button type="button" class="btn btn-dark btn-sm" data-toggle="modal" data-target="#takeaction">Take Action</button>
<?php 
endif;
endif;?>    
                                </div>
...
<?php while($result=mysqli_fetch_array($ret)){?>
    ...
    <td><?php echo $result['FullName'];?>-(<?php echo $result['EmpID'];?>)</td>
    </tr>
...
<form method="post">
        ...
        <?php } ?>
            ...
            <p>
     <input type="submit" class="btn btn-dark btn-user btn-block" name="submit" id="submit">   </p> 
...
<form method="post" enctype="multipart/form-data" >
  ...
  $query1=mysqli_query($con,"select ReportStatus from tblvacrecords where OrderNumber='$orderid'");
